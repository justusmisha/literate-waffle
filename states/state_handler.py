...
from utils.functions import prompt_maker_en
...
async def first_cat(message: types.Message, state: FSMContext):
    text = prompt_maker_en(deep_translator(message.text.split()[0], 'ru', 'en'), deep_translator("Личные вещи", 'ru','en'), deep_translator(message.text,'ru', 'en'))
...
async def fifth_cat(message: types.Message, state: FSMContext):
    text = prompt_maker_en(deep_translator(message.text.split()[0], 'ru', 'en'), deep_translator("Услуги", 'ru','en'), deep_translator(message.text,'ru', 'en'))
    await bot.send_message(chat_id=message.chat.id,
                           text=meta_ai(text))
    await state.finish()
...
async def sixth_cat(message: types.Message, state: FSMContext):
    text = prompt_maker_en(deep_translator(message.text.split()[0], 'ru', 'en'), deep_translator("Для дома и дачи", 'ru','en'), deep_translator(message.text,'ru', 'en'))
    await bot.send_message(chat_id=message.chat.id,
                           text=meta_ai(text))
    await state.finish()
...
async def seventh_cat(message: types.Message, state: FSMContext):
    text = prompt_maker_en(deep_translator(message.text.split()[0], 'ru', 'en'), deep_translator("Бытовая электроника", 'ru','en'), deep_translator(message.text,'ru', 'en'))
    await bot.send_message(chat_id=message.chat.id,
                           text=meta_ai(text))
    await state.finish()
